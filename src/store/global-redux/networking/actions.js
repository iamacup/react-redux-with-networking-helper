...
// data is a JSON object of any data to go along with the request
// caller is a unique ID (string) that identifies what function is calling the method
// onlyOne when true will ensure that the state only contains the LATEST response for path/caller combination
export function initiateNetworkRequest(path, method, data, caller, onlyLatest = true) {
  return {
    type: 'GLOBAL_NETWORK_REQUEST_INITIATE',
    path,
    method,
...
export function saveAuthToken(token) {
  return {
    type: 'GLOBAL_NETWORK_SAVE_TOKEN',
    token,
  };
}

export function clearDataWithDataSubscriptionCaller(dataSubscriptionCaller) {
  return {
    type: 'GLOBAL_NETWORK_REQUEST_CLEAR_DATA_WITH_CALLER',
    caller: dataSubscriptionCaller,
  };
}

export function clearAll() {
  return {
    type: 'GLOBAL_NETWORK_REQUEST_CLEAR',
  };
}

export function initiateNetworkRequestLazy(path, method, data) {
  return {
    type: 'GLOBAL_NETWORK_REQUEST_INITIATE_LAZY',
    path,
    method,
    data,
    startTimestamp: Date.now(),
...
export function networkLazyRetry(data) {
  return {
    ...data,
    type: 'GLOBAL_NETWORK_REQUEST_LAZY_RETRY',
  };
}

export function networkResponseLazyNotComplete(uuid) {
  return {
    type: 'GLOBAL_NETWORK_REQUEST_RESPONSE_LAZY_NOT_COMPLETE',
    uuid,
  };
}

export function networkResponseLazyComplete(uuid) {
  return {
    type: 'GLOBAL_NETWORK_REQUEST_RESPONSE_LAZY_COMPLETE',
    uuid,
  };
}
*/
